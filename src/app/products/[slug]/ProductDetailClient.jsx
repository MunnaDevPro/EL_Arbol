...
import { ShoppingCart, Check, ChevronLeft, Share2, Heart, Truck, Store, Minus, Plus, ShoppingBag } from 'lucide-react'
      ...
      <div className="flex border-b border-[#BCCAC1]/30 overflow-x-auto" style={{ scrollbarWidth: 'none' }}>
        {tabs.map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
            className="shrink-0 px-5 py-4 text-sm transition-colors"
            style={{
                ...
                <ul className="space-y-2">
                  {product.keyFeatures.map((f) => (
                    <li key={f} className="flex items-start gap-2" style={{ fontSize: '14px', color: '#3D4943' }}>
                      <span className="mt-1.5 w-1.5 h-1.5 rounded-full bg-[#00694C] shrink-0" />{f}
                    </li>
                  ))}
                </ul>
              </div>
                ...
                <ul className="space-y-2">
                  {product.bestUsedFor.map((u) => (
                    <li key={u} className="flex items-start gap-2" style={{ fontSize: '14px', color: '#3D4943' }}>
                      <span className="mt-1.5 w-1.5 h-1.5 rounded-full bg-[#00694C] shrink-0" />{u}
                    </li>
                  ))}
                </ul>
              </div>
    ...
    <section className="border-t border-[#BCCAC1]/30 pt-12 pb-24 lg:pb-20">
      <div className="flex items-end justify-between mb-8">
        <h2 style={{
          ...
          fontWeight: 700, color: '#151E13',
        }}>
          You might also enjoy
        </h2>
        <Link href="/" className="hidden sm:flex items-center gap-1 hover:underline"
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-5">
        {related.map((p) => (
          <Link key={p.id} href={`/products/${slugify(p.name)}`}
            className="group block bg-white rounded-2xl overflow-hidden border border-gray-100 hover:shadow-md hover:border-[#BCCAC1]/60 transition-all duration-200">
            ...
            <div className="p-3.5">
              ...
              <p className="text-[12px] text-[#6D7A73] mb-2">{p.category} · {p.origin}</p>
              <p className="font-bold text-[15px] text-amber-500">
                €{p.price.toFixed(2)}
                <span className="text-[12px] font-normal text-gray-400"> /{p.unit?.split(' ')[0]}</span>
              </p>
            </div>
          </Link>
        ))}
...
export default function ProductDetailClient({ product, related }) {
  ...
  return (
    <main className="bg-[#FAFAF8] min-h-screen">
      ...
      {/* ── Breadcrumb ── */}
      <div className="max-w-[1280px] mx-auto px-6 lg:px-10 pt-5">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16">
              ...
              <div className="absolute top-4 right-4 flex items-center gap-2">
                ...
                <button onClick={() => setWishlisted(!wishlisted)}
                  ...
                  <Heart size={15} fill={wishlisted ? '#ef4444' : 'none'}
                    className={wishlisted ? 'text-red-500' : 'text-[#3D4943]'} />
                </button>
              </div>
          ...
          <div className="flex flex-col gap-5">
            ...
            <h1 style={{
              ...
              fontWeight: 700, color: '#151E13', lineHeight: 1.15,
            }}>
              {product.name}
            </h1>
            ...
            <div className="flex items-center gap-3 flex-wrap">
              <span style={{
                fontFamily: '"Playfair Display", Georgia, serif',
                fontSize: '2rem', fontWeight: 700, color: '#151E13',
              }}>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <div className="relative">
                  <select className="w-full appearance-none border border-[#BCCAC1]/60 rounded-xl px-4 py-3 text-sm text-[#151E13] bg-white pr-9 focus:outline-none focus:border-[#00694C]"
            ...
            <div>
              ...
              <div className="space-y-2">
                {[
                  { id: 'delivery', label: 'Home Delivery', sub: 'Today before 6:00 PM', icon: <Truck size={18} className="text-[#00694C]" /> },
                  { id: 'collect', label: 'Click & Collect', sub: 'Ready in 2 hours', icon: <Store size={18} className="text-[#6D7A73]" /> },
                ].map((opt) => (
                  <label key={opt.id}
                    ...
                    <input type="radio" name="fulfillment" value={opt.id}
                      checked={fulfillment === opt.id} onChange={() => setFulfillment(opt.id)}
            ...
            {product.inStock ? (
              <button onClick={handleAddToCart}
                ...
                style={{ background: added ? '#085041' : '#00694C', color: 'white', fontSize: '15.5px', fontWeight: 600 }}>
                {added ? <><Check size={19} />Added to Cart</> : <><ShoppingCart size={19} />Add to Cart</>}
              </button>
            ) : (
              <button disabled
            ...
            {product.inStock && (
              <p className="text-center" style={{ fontSize: '12.5px', color: '#00694C' }}>
                ...
                In Stock — 42 units available
              </p>
            )}

            {/* Trust strip */}
            <div className="grid grid-cols-3 gap-2 pt-2 border-t border-[#BCCAC1]/20">
              {[{ icon: '✦', label: 'Fresh Daily' }, { icon: '↗', label: 'Free > €40' }, { icon: '🔒', label: 'Secure Pay' }].map((t) => (
